...
- **Ao mudar `collection_id` de qualquer entidade, escreva a migração de religamento junto.** A unificação da grade com o cronograma deixou 112 referências apontando para um id que não tinha mais cronograma: os dados continuaram no banco e sumiram de todas as telas (migração v6→v7 corrigiu, achando a coleção-alvo pelos `phase_deadlines`). Regenerar o `data.js` não conserta quem já tem banco local — só a migração conserta.
...
- **Catálogos do produto** (`db.catalogos = {tipos, cores, tamanhos, tecidos}`) são listas de **nomes**, não ids: a referência guarda o nome, então renomear no catálogo nunca quebra o cadastro de uma peça e o export sai legível. Editados em `#/cadastros` (`vCadastros`); `catalogoEmUso()` avisa antes de remover.
- **Campos descritivos da referência** (`tipo_produto`, `tecido_principal`, `cores[]`, `tamanhos[]`, `foto`) são digitados na plataforma e **não vêm da planilha** — preservados no `applyNewSeed` por `guardarDescritivosRef`/`reaplicarDescritivos`, chaveados por `collection_id|code`. Junto com os catálogos, que também são do time.
- **Foto é reduzida antes de gravar** (`fotoReduzida`, 320px / JPEG 0.72 ≈ 15 KB): o localStorage tem ~5 MB e guardar o arquivo original estouraria em poucas peças. `usoArmazenamento()` mostra o consumo no modal de Dados.
- **Indicador de tecido**: `tecidosAgregados()` conta as referências ativas da coleção por `tecido_principal` e mostra "— não informado —" explicitamente, com aviso de que o percentual só fecha quando todas tiverem tecido. Nunca omitir o não-informado da base: o percentual ficaria inflado.
...
- **"Nada se moveu" tem três causas distintas e a tela precisa dizer qual** (`htmlSimulacao`): (1) nada depende dela; (2) há margem — `limiarImpacto()` diz a partir de quantos dias começa a empurrar; (3) a cadeia à frente passa por etapa sem data e o cálculo não fecha. **Checar a margem ANTES da falta de data**: o caminho inverso culpa o dado faltante quando na verdade havia folga de sobra, e o time vai atrás do problema errado. Quando há etapas sem data na cadeia, o aviso entra como complemento mesmo quando o impacto é calculável — o alcance real pode ser maior.
...
- **"Por que o marco atrasou" (`origemAtraso`)**: anda para trás a partir da liberação do PCP pelo predecessor que **manda hoje** (maior fim projetado) e devolve a fila com o quanto cada etapa somou (`somou = desvio − acumulado anterior`, nunca negativo). O acumulado da última etapa é, por construção, o atraso do marco. Quando uma etapa terminou **sem esperar a anterior** (acumulado cai mais de 7 dias — comum no realizado), ela entra em `foraDeOrdem` e a tela avisa que os saltos não somam o total: apresentar uma soma que não bate destruiria a confiança no número. A lista "o que dá para fazer agora" traz só as etapas **em aberto que estão somando** atraso — a fila inteira dilui e o time não sabe por onde começar.
